...
class ProfileTableViewController: UITableViewController {
    ...
    var autogeneratedNickname = Nicknames()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == Sections.Statistics.rawValue {
            ...
            content.textProperties.color = .systemPurple
            content.textProperties.alignment = .center
            content.text = changeNickname[indexPath.row]
            cell.contentConfiguration = content
            return cell
    ...
    func changeNickname(_ indexPath: IndexPath) {
        if indexPath.row == 0 {
            let newNickname = autogeneratedNickname.generateNickname()
            let changeRequest = user?.createProfileChangeRequest()
            changeRequest?.displayName = newNickname
            changeRequest?.commitChanges { (error) in
                guard error == nil else {
                    print("Could not change displayName")
                    return
                }
                self.changeNicknameInDatabase(newNickname)
                self.tableView.reloadData()
            }
        } else {
            let alert = UIAlertController(
                title: "Change personal information",
                message: changeNickname[indexPath.row],
            ...
            let actionUpdate = UIAlertAction(
                ...
                style: .default) { action in
                    guard let newNickname = alert.textFields?.first?.text as? String else {
                        return
                    }
    ...
    func changeNicknameInDatabase(_ newNickname: String) {
        guard let userId = self.user?.uid as? String else {
            return
        }
        self.database.collection("users").document("\(userId)").setData(["nickname":newNickname]) { err in
            if let err = err {
